...
public class StringUtils {
	...
	public static String fillZero(String value, int len) {
		if (StringUtils.isNull(value) || len <= 0) {
	...
	public static String couponIdGenerator(String src) {
		...
		for (int i = 0; i < srcTemp.length(); i++) {
			iSrc = iSrc + (250 - 1 - i) * (srcTemp.charAt(i));
		}
	 ...
	 * 
	 * @param Object
	 *            [] 例如： 传入: Object[]参数: String[] result={"TYHR0001","TYHR0002"}
	 *            delim参数: "," 返回： "'TYHR0001','TYHR0002'"
	 * 
	 * @param Object
	 *            []
	 * @return String
	...
	/**
	 * 在字符串 s 中，每隔 insertUnit 长度插入 字符separator
	 */
	public static String insertSeparatorEveryUnit2(String s, int insertUnit, char separator) {
		int length;
		if (null == s || (length = s.length()) == 0) {
			throw new RuntimeException("字符串s不能为空");
		}

		if (insertUnit <= 0) {
			throw new RuntimeException("insertUnit 是每隔多少字符插入的单元，不能小于或等于0");
		}

		StringBuilder sb = new StringBuilder(s);

		for (int i = 0; i < (length / insertUnit); i++) {
			int k = (i + 1) * insertUnit + i;
			sb.insert(k, separator);
		}
