...
import com.zcbspay.platform.support.task.enums.ChannelEnmu;
import com.zcbspay.platform.support.task.enums.TradeQueueEnum;
import com.zcbspay.platform.support.task.enums.TradeStatFlagEnum;
					...
					if (ChannelEnmu.CMBCWITHHOLDING==channelEnmu) {
						...
						try {
							cmbcWithholdingProducer.sendJsonMessage(JSON.toJSONString(tradeQueueBean), WithholdingTagsEnum.WITHHOLDING_QUERY_ACCOUNTING);
								...
								| InterruptedException | MQBrokerException | com.alibaba.rocketmq.remoting.exception.RemotingException e) {
							e.printStackTrace();
							tradeQueueService.addTradeQueue(tradeQueueBean);
						}*/
